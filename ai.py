...
class AI:
    def __init__(self, model_name="gpt-4", temperature=0.1):
        ...
        self.conversation = []  # List to store conversation history
    ...
    def get_response_to_prompt(self, prompt: str) -> str:
        ...
        # Generate text using GPT-3.5 chat model with conversation history
        response = self.llm(messages=self.conversation, max_tokens=150)

        # Get the AI's response from the AIMessage object's content attribute
        generated_text = response.content.strip()

        # Add AI response to the conversation
        self.add_to_conversation("ai", generated_text)

        self.update_token_usage_log(
            messages=[self.fuser(prompt), self.fassistant(generated_text)],
            answer=generated_text,
            step_name="Prompt-Response",
        )
